...
syn match vimCommand_ contained "n\%(\%[ext]\|ew\)\>" nextgroup=vimBang
...
" :chdir and :delete is handled specially elsewhere
syn match vimCommand  contained "\<co\%[py]\>"     nextgroup=vimBang
syn match vimCommand_ contained   "co\%[py]\>"     nextgroup=vimBang
syn match vimCommand  contained "\<j\%[oin]\>"     nextgroup=vimBang
syn match vimCommand_ contained   "j\%[oin]\>"     nextgroup=vimBang
syn match vimCommand  contained "\<sp\%[lit]\>"    nextgroup=vimBang
syn match vimCommand_ contained   "sp\%[lit]\>"    nextgroup=vimBang
syn match vimCommand  contained "\<sw\%[apname]\>" nextgroup=vimBang

" commands starting with uppercase letter
syn keyword vimCommand  contained N[ext]       nextgroup=vimBang
syn match   vimCommand_ contained "N\%[ext]\>" nextgroup=vimBang
syn keyword vimCommand  contained P[rint]
syn match   vimCommand_ contained "P\%[rint]\>"
syn keyword vimCommand  contained X

" GEN_SYN_VIM: vimCommand modifier, START_STR='syn keyword vimCommandModifier contained', END_STR='skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func'
...
syn match vimCommandModifier_ contained "hid\%[e]\>" skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func
syn match vimCommandModifier_ contained "tab\>" skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func
syn match vimCommandModifier_ contained "verb\%[ose]\>" skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func
" modifiers supporting bang, :filter is handled specially elsewhere
syn keyword vimCommandModifier contained sil[ent] skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func

syn match	vimCommandModifierBang	contained	"\a\@1<=!"	skipwhite skipnl nextgroup=@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func

" lower priority :syn-match to allow for :command/function() distinction
syn match vimCommandModifier contained "\<bro\%[wse]\>"  skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func
syn match vimCommandModifier contained "\<conf\%[irm]\>" skipwhite skipnl nextgroup=vimCommandModifierBang,@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func

syn match	vimCommandModifierContinue	contained	"^\s*\\"        skipwhite skipnl nextgroup=@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func	contains=vimLeadingWhitespace
...
syn match	vimUserAutoEvent	contained	"\<\h\w*\>"	skipwhite skipnl nextgroup=vimUserAutoEventSep,vimAutocmdMod,vimAutocmdBlock,@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc
...
syn keyword vimFuncName contained histdel histget histnr hlID hlexists hlget hlset hostname iconv id indent index indexof input inputdialog inputlist inputrestore inputsave inputsecret insert instanceof interrupt invert isabsolutepath isdirectory isinf islocked isnan items job_getchannel job_info job_setoptions job_start job_status job_stop join js_decode js_encode json_decode json_encode keys keytrans len libcall libcallnr line line2byte lispindent list2blob list2str list2tuple listener_add listener_flush listener_remove localtime log log10 log2 luaeval map maparg mapcheck maplist mapnew mapset match matchadd matchaddpos matcharg matchbufline matchdelete matchend matchfuzzy matchfuzzypos matchlist matchstr matchstrlist matchstrpos max menu_info min mkdir mode mzeval
...
syn match	vimCmdStart	contained	":\+"	skipwhite nextgroup=vimCmdStart,@vimRange,@vimCmdList,vimComment
syn match	vimCount	contained	"\d\+"
syn match	vimBang	contained	"!"
syn match	vimPrintFlag	contained	"[lp#]"	skipwhite nextgroup=vimPrintFlag,vimComment,vimCmdSep

syn match	vimWhitespace	   contained	"\s\+"
syn match	vimLeadingWhitespace contained	"^\s\+"

" :range {{{3
syn match	vimRange	contained	"[*%]" skipwhite nextgroup=vimAddressOffset,vimAddressSeparator,vimCmdStart,@vimCmdList,vimCommand

syn region	vimAddressPattern	contained
      \ matchgroup=Delimiter
      \ start="/"
      \ skip=+\\/\|\n\s*\%(\\\|["#]\\ \)+
      \ end="$"
      \ end="/"
      \ skipwhite nextgroup=vimAddressOffset,vimAddressSeparator,vimCmdStart,@vimCmdList,vimCommand
      \ contains=vimAddressSlashEscape,@vimContinue,@vimSubstList
syn match	vimAddressSlashEscape	contained	"\\/"
syn region	vimAddressPattern	contained
      \ matchgroup=Delimiter
      \ start="?"
      \ skip=+\\?\|\n\s*\%(\\\|["#]\\ \)+
...
" :number, :# {{{3
" ===========
" TODO: needs to be limited to after a ':' in Vim9, otherwise a comment
if s:vim9script
  syn match	vimPound	contained	":\@1<=#"	skipwhite nextgroup=vimPoundCount,vimPrintFlag
else
  syn match	vimPound	contained	"#"	skipwhite nextgroup=vimPoundCount,vimPrintFlag
endif
syn keyword	vimPound	contained	nu[mber]	skipwhite nextgroup=vimPoundCount,vimPrintFlag
syn match	vimPound_	contained	"nu\%[mber]\>"	skipwhite nextgroup=vimPoundCount,vimPrintFlag
syn match	vimPoundCount	contained	"\d\+"	skipwhite nextgroup=vimPrintFlag

" := {{{3
" ==
syn match	vimEquals	contained	"="	skipwhite nextgroup=vimPrintFlag

" :>, :< {{{3
" ======

syn match	vimShiftLeft	 contained	"<\%(line\)\@!"	skipwhite nextgroup=vimShiftLeftPlus,vimShiftLeftCount,vimPrintFlag
syn match	vimShiftLeftPlus	 contained	"<"	skipwhite nextgroup=vimShiftLeftPlus,vimShiftLeftCount,vimPrintFlag
syn match	vimShiftLeftCount	 contained	"\d\+"	skipwhite nextgroup=vimPrintFlag

syn match	vimShiftRight	 contained	">"	skipwhite nextgroup=vimShiftRightPlus,vimShiftRightCount,vimPrintFlag
...
" remove
syn match	vimAugroup	contained	"\<aug\%[roup]!"	skipwhite nextgroup=vimAugroupName contains=vimAugroupKey,vimAugroupBang
...
syn region	vimAutocmdPattern	contained
      ...
      \ end="\ze[,[:space:]]"
      \ end="$"
      \ skipwhite skipnl nextgroup=vimAutocmdPatternSep,vimAutocmdMod,vimAutocmdBlock,@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc
      \ contains=vimEnvvar,@vimWildcard,vimAutocmdPatternEscape
syn match	vimAutocmdBufferPattern	contained	"<buffer\%(=\%(\d\+\|abuf\)\)\=>" skipwhite skipnl nextgroup=vimAutocmdPatternSep,vimAutocmdMod,vimAutocmdBlock,@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc
...
syn match	vimAutocmdPatternSep	contained	",\ze\s\+"	                skipwhite skipnl nextgroup=vimAutocmdMod,vimAutocmdBlock,@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc
...
syn match	vimAutocmdMod	contained	"\%(++\)\=\<nested\>" skipwhite skipnl nextgroup=vimAutocmdMod,vimAutocmdBlock,@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc
syn match	vimAutocmdMod	contained	"++once\>"	    skipwhite skipnl nextgroup=vimAutocmdMod,vimAutocmdBlock,@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc
...
syn match	vimAutocmdContinue	contained	"^\s*\\"        skipwhite skipnl nextgroup=@vimAutocmdContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func,@vimFunc	contains=vimLeadingWhitespace
...
syn keyword	vimBreakaddHere	contained	here	skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep
syn keyword	vimBreakaddExpr	contained	expr	skipwhite nextgroup=@vimExprList

syn match	vimBreakpointGlob		contained	"*"	skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep
syn match	vimBreakpointNumber	contained	"\<\d\+\>"	skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep

syn cluster vimBreakpointArg contains=vimBreakaddFunc,vimBreakaddFile,vimBreakaddHere,vimBreakaddExpr

syn match	vimBreakpointFunction	contained	"\<\%(\*\|\w\)\+\>"	skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep
syn match	vimBreakpointFilename	contained	"\<\%(\*\|\f\)\+\>"	skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep
...
syn keyword	vimBreakadd	contained	breaka[dd]	skipwhite nextgroup=@vimBreakpointArg
syn keyword	vimBreakdel	contained	breakd[el]	skipwhite nextgroup=@vimBreakpointArg,vimBreakpointNumber,vimBreakpointGlob
syn keyword	vimBreaklist	contained	breakl[ist]	skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep

" :buffer {{{3
" =======
" lower priority than [wtb]: Vim9 scoped variables at SOL
syn match	vimBuffer	contained	"\<b\%[uffer]\>"	nextgroup=vimBang
syn match	vimBuffer_	contained	"b\%[uffer]\>"	nextgroup=vimBang
...
" :cd {{{3
" ===
" GEN_SYN_VIM: vimCommand cd, START_STR='syn keyword vimCd contained', END_STR='skipwhite nextgroup=vimCdBang,vimCdArg,vimComment,vim9Comment,vimCmdSep'
syn keyword vimCd contained cd lc[d] lch[dir] tc[d] tch[dir] skipwhite nextgroup=vimCdBang,vimCdArg,vimComment,vim9Comment,vimCmdSep
syn match	vimCd	contained	"\<chd\%[ir]\>"	skipwhite nextgroup=vimCdBang,vimCdArg,vimComment,vim9Comment,vimCmdSep
...
" :command {{{3
" ========
syn cluster	vimUserCmdList	contains=@vimCmdList,vimComment,vimCtrlChar,vimEscapeBrace,@vimFunc,vimNotation,vimNumber,vimOper,vimSpecFile,vimString,vimSubst,vimSubstRep,vimSubstRange,@vim9Lhs,vimCmdSep
...
syn match	vimCopy_	contained	"t\>"	nextgroup=vimBang
...
" =======
" TODO: handle Vim9
syn keyword	vimDelete	contained	d[elete]	skipwhite nextgroup=vimDeleteRegister,vimDeleteCount,vimPrintFlag
syn match	vimDelete_	contained	"d\%[elete]\>"	skipwhite nextgroup=vimDeleteRegister,vimDeleteCount,vimPrintFlag

syn match	vimDelete	contained	"\<d\%[elete]\ze\%([lp]\+\>\|#\)"	nextgroup=vimPrintFlag
syn match	vimDelete_	contained	  "d\%[elete]\ze\%([lp]\+\>\|#\)"	nextgroup=vimPrintFlag
" :del is :delete only, not :delete | :list
syn match	vimDelete	contained	"\<del\>"
syn match	vimDelete_	contained	  "del\>"

syn match	vimDelete	contained	"\<d\%[elete]\ze\%(_\|\d\+\>\)"	nextgroup=vimDeleteRegister,vimDeleteCount
syn match	vimDelete_	contained	  "d\%[elete]\ze\%(_\|\d\+\>\)"	nextgroup=vimDeleteRegister,vimDeleteCount

syn match	vimDeleteRegister	contained	+[-*+_]\|\\"+	skipwhite nextgroup=vimDeleteCount,vimPrintFlag
syn match	vimDeleteRegister	contained	+\<\a+	skipwhite nextgroup=vimDeleteCount,vimPrintFlag

syn match	vimDeleteCount	contained	"\d\+"	skipwhite nextgroup=vimPrintFlag
...
syn region	vimExFilterPattern contained
      \ start="[[:ident:]]"
      \ end="\ze[[:space:]\n]"
      \ skipwhite skipnl nextgroup=@vimCommandModifierContinue,vimCmdStart,@vimRange,@vimCmdList,@vim9Lhs,@vim9Func
...
" =======
if s:vim9script
  syn keyword	vim9Export	export	contained
        \ skipwhite nextgroup=vim9Abstract,vim9ClassBody,vim9Const,vimDef,vimDefFold,vim9EnumBody,vim9Final,vim9InterfaceBody,vim9Type,vim9Var
endif
...
syn match	vim9ForInComment		contained	"#.*"	skipwhite skipempty nextgroup=vim9ForInComment,@vimExprList
...
" ============================
syn keyword	vimHelp	contained	h[elp]	skipwhite nextgroup=vimHelpBang,vimHelpArg,vimHelpEndCommand
...
syn keyword	vimHelpgrep	 contained	helpg[rep] lh[elpgrep]	skipwhite nextgroup=vimHelpgrepPattern
...
syn keyword	vimHistory	contained	his[tory]	skipwhite nextgroup=vimHistoryName,vimHistoryRange,vimCmdSep,vimComment,vim9Comment
...
syn keyword	vimImport	contained	imp[ort]		skipwhite nextgroup=vimImportAutoload,vimImportFilename

" :language {{{3
" =========
syn keyword 	vimLanguage		contained	lan[guage]		skipwhite nextgroup=@vimLanguageName,vimLanguageCategory,vimCmdSep,vimComment,vim9Comment
...
syn match	vimLanguageName		contained	"[[:alnum:]][[:alnum:]._@-]*[[:alnum:]]"	nextgroup=vimCmdSep,vimComment,vim9Comment
...
syn match	vimMark_	contained	"ma\%[rk]\>"	      	skipwhite nextgroup=@vimMarkArg
...
" :packadd {{{3
" ========
syn keyword	vimPackadd	contained	pa[ckadd]	skipwhite nextgroup=vimPackaddBang,vimPackaddArg,vimComment,vim9Comment,vimCmdSep
syn region	vimPackaddArg	contained
      \ start=+["#|]\@!\S+
      \ end="\ze\s*$"
      \ end=+\ze\s*\\\@1<!["#|]+
      \ skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep
      \ contains=vimSpecFile,@vimWildcard
      \ oneline
syn match	vimPackaddBang	contained	"\a\@1<=!"	skipwhite nextgroup=vimPackaddArg
...
syn region	vimPromptArg	contained
      ...
      \ end="$"
      \ contains=@vimContinue
syn keyword	vimPrompt	contained	promptf[ind] promptr[epl]	skipwhite nextgroup=vimPromptArg
...
" :runtime {{{3
" ========
syn keyword	vimRuntime	contained	ru[ntime]	skipwhite nextgroup=vimRuntimeBang,vimRuntimeWhere,vimRuntimeArgs
syn keyword	vimRuntimeWhere	contained
      \ START OPT PACK ALL
      \ skipwhite nextgroup=vimRuntimeArgs
syn region	vimRuntimeArgs	contained
      \ start=+["#|]\@!\S+
      \ skip=+\s*\n\s*\%(\\\|["#]\\ \)\|^\s*["#]\\ +
      \ end="\ze\s*$"
      \ end=+\ze\s*\\\@1<!["#|]+
      \ skipwhite nextgroup=vimComment,vim9Comment,vimCmdSep
      \ contains=@vimContinue,vimSpecFile,@vimWildcard
syn match	vimRuntimeBang	contained	"\a\@1<=!"	skipwhite nextgroup=vimRuntimeWhere,vimRuntimeArgs
...
syn match	vimSort_	  contained	"sort\=\>"		skipwhite nextgroup=vimSortBang,@vimSortOptions,vimSortPattern,vimCmdSep
...
syn keyword	vimSyntime	contained	synti[me]		skipwhite nextgroup=vimSyntimeArg
...
syn match	vimTerminal_	contained	"ter\%[minal]\>"			skipwhite        nextgroup=vimTerminalOptions,vimTerminalCommand
syn match	vimTerminal_	contained	+ter\%[minal]\>\ze\s*\n\s*\%(\\\|["#]\\ \)+	skipwhite skipnl nextgroup=vimTerminalOptions,vimTerminalCommand,@vimTerminalContinue
...
" NOTE: handled with higher priority later in file
...
syn match	vimWrite_	contained	"w\%[rite]\>"	nextgroup=vimBang
...
if s:vim9script
  ...
  syn keyword	vim9Abstract		contained	abstract	skipwhite skipnl nextgroup=vim9ClassBody,vim9AbstractDef
  ...
  VimFolde syn region	vim9EnumBody	contained
        \ start="\<enum\>"
        \ matchgroup=vimCommand
        \ end="\<endenum\>"
        \ contains=@vim9EnumBodyList
        \ transparent
  ...
  VimFoldi syn region	vim9InterfaceBody	contained
        \ start="\<interface\>"
        \ matchgroup=vimCommand
        \ end="\<endinterface\>"
        \ contains=@vim9InterfaceBodyList
        \ transparent

  " Type Aliases {{{3
  syn keyword	vim9Type	 contained	ty[pe]	skipwhite nextgroup=vim9TypeAlias,vim9TypeAliasError
...
syn region	vimContinueString	contained	matchgroup=vimContinueString start=+"+  skip=+\n\s*\%(\\\|["#]\\ \)+ end=+"+ end="$" skipwhite nextgroup=vimSubscriptBrackets,vimComment,vimOper contains=@vimContinue,@vimStringGroup
syn region	vimContinueString	contained	matchgroup=vimContinueString start=+'+  skip=+\n\s*\%(\\\|["#]\\ \)+ end=+'+ end="$" skipwhite nextgroup=vimSubscriptBrackets,vimComment,vimOper contains=@vimContinue,vimQuoteEscape
syn region	vimContinueString	contained	matchgroup=vimContinueString start=+$"+ skip=+\n\s*\%(\\\|["#]\\ \)+ end=+"+ end="$" skipwhite nextgroup=vimSubscriptBrackets,vimComment,vimOper contains=@vimContinue,@vimStringInterpolation,@vimStringGroup
syn region	vimContinueString	contained	matchgroup=vimContinueString start=+$'+ skip=+\n\s*\%(\\\|["#]\\ \)+ end=+'+ end="$" skipwhite nextgroup=vimSubscriptBrackets,vimComment,vimOper contains=@vimContinue,@vimStringInterpolation,vimQuoteEscape
...
syn region	vim9VariableTypeObjectArgs	contained
...
" Compiler plugins
syn keyword	vimCompilerSet	contained	CompilerSet	skipwhite nextgroup=vimSetArgs

" runtime/makemenu.vim
syn keyword	vimSynMenu	contained	SynMenu	skipwhite nextgroup=vimSynMenuPath
...
syn keyword	vimSynColor	contained	SynColor	skipwhite nextgroup=vimSynColorGroup
...
syn match	vimTcl_	contained	"tcl\>"	skipwhite nextgroup=vimTclHeredoc,vimTclStatement
syn match	vimTcl_	contained	"tcld\%[o]\>"	skipwhite nextgroup=vimTclStatement
syn match	vimTcl_	contained	"tclf\%[ile]\>"
...
syn match vimCmdSep	"\\\@1<!|\%(\%(\s*\n\s*["#]\\ .*\)*\s*\n\s*\\\)\="	skipwhite nextgroup=vimCmdStart,@vimRange,@vimCmdList,@vimFunc,@vim9Func,vim9Block,vim9Lhs.*  contains=@vimCmdSepContinue
...
Vim9 syn match	vim9LhsVariable_	"^\s*\h\w*\ze[[.]"			          nextgroup=vimSubscriptBrackets,vimSubscriptDot contains=vimVar,vim9Super,vim9This,vimWhitespace
...
Vim9 syn match	vim9UserFunc	"^\s*\%([sgbwtv]:\|<[sS][iI][dD]>\)\=\h\w*\ze[(<]"		skipwhite nextgroup=vimOperParen,vim9TypeArgs	   contains=vimVarScope,vimNotation,vim9MethodName,vim9Super,vim9This,vimWhitespace
...
if !exists("skip_vim_syntax_inits")
 ...
 hi def link vimBuffer_	vimBuffer
 ...
 hi def link vimDelete	vimCommand
 hi def link vimDelete_	vimDelete
 hi def link vimDeleteCount	Number
 ...
 hi def link vimTextInputBang	vimBang
 hi def link vimTextInputComment	vimComment
 hi def link vimTextInputEnd	vimCommand
 hi def link vimTextInputText	String
 ...
 hi def link vimEquals	vimCommand
 ...
 hi def link vimPackadd	vimCommand
 hi def link vimPackaddBang	vimBang
 ...
 hi def link vimPound	vimCommand
 hi def link vimPound_	vimPound
 hi def link vimPoundCount	Number
 hi def link vimPrintFlag	Special
 ...
 hi def link vimRuntime	vimCommand
 hi def link vimRuntimeBang	vimBang
 hi def link vimRuntimeWhere	Special
 ...
 hi def link vimShiftLeftCount	Number
 hi def link vimShiftLeftPlus	Special
 hi def link vimShiftLeft	vimCommand
 hi def link vimShiftRightCount	Number
 hi def link vimShiftRightPlus	Special
 hi def link vimShiftRight	vimCommand
 ...
 hi def link vimTerminal_	vimCommand
 ...
 hi def link vimWrite_	vimWrite
